...
// --- 核心改动：使用 express.static 提供打包到.exe内部的整个 public 文件夹 ---
// __dirname 在 pkg 打包的应用中会指向 snapshot 虚拟文件系统
...
app.listen(PORT, async () => {
    try {
        await fs.mkdir(DATA_DIR, { recursive: true });
        await writeLog('服务器启动成功 (生产模式)。');
        console.log(`服务器正在运行，请在浏览器中访问 http://localhost:${PORT}`);
        console.log(`应用数据和日志将存储在: ${APP_DIR}`);
    } catch (error) {
        console.error('初始化应用目录失败:', error); await writeLog(`初始化应用目录失败: ${error.stack}`, 'error');
    }
});